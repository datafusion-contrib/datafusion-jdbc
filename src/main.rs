...
use crate::args::{extract_memory_pool_size, JDBCConfig, PoolType};
use args::Args;
    ...
    async fn create_ctx(&self) -> Result<String, Status> {
        ...
        let config =
            ...
            if let Some(memory_limit) = &self.conf.memory_limit {
                ...
                if let Some(mem_pool_type) = &self.conf.mem_pool_type {
                    match mem_pool_type {
                        PoolType::Greedy => config
                            .with_memory_pool(Arc::new(GreedyMemoryPool::new(memory_limit))),
                        PoolType::Fair => config
        ...
        let runtime_env = Arc::new(RuntimeEnv::new(config).unwrap());
        let session_config = SessionConfig::from_env()
            .map_err(|e| Status::internal(format!("Error building plan: {e}")))?
            .with_information_schema(true)
            .with_batch_size(8192);
        let ctx = Arc::new(SessionContext::with_config_rt(
            session_config,
            runtime_env.clone(),
        ));

        let parsed_path = ListingTableUrl::parse(&self.conf.delta_path).unwrap();
        let object_store = runtime_env
            .object_store(parsed_path.object_store())
            .unwrap();
        ...
        for table_name in table_list {
            let table_path = format!("{}/{}", &self.conf.delta_path, &table_name);
            let table = deltalake::open_table(&table_path).await.unwrap();
            ctx.register_table(&table_name, Arc::new(table)).unwrap();
        }

        info!("Finished registering delta tables into DataFusion context");
